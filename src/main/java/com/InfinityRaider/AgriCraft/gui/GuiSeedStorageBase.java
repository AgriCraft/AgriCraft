...
public abstract class GuiSeedStorageBase extends GuiContainer {
	...
	private void sortByStat(List<SeedStorageSlot> list) {
		...
		switch (this.sortStatId) {
			case buttonIdGrowth:
			stat = Names.NBT.growth;
			break;
			case buttonIdGain:
			stat = Names.NBT.gain;
			break;
			case buttonIdStrength:
			stat = Names.NBT.strength;
			break;
		}
	...
	protected void drawActiveEntries(int x, int y, ResourceLocation texture, int xOffset, int yOffset) {
		...
		for (int i = 0; i < this.activeSeeds.size(); i++) {
			...
			stack.stackTagCompound = SeedHelper.setNBT(new NBTTagCompound(), stats.growth, stats.gain, stats.strength, true);
			...
			this.drawTexturedModalRect(this.guiLeft + xOffset + (i * 16) + 1, this.guiTop + yOffset - stats.growth, 0, textureSize - stats.growth, 3, stats.growth);
			this.drawTexturedModalRect(this.guiLeft + xOffset + i * 16 + 6, this.guiTop + yOffset - stats.gain, 0, textureSize - stats.gain, 3, stats.gain);
			this.drawTexturedModalRect(this.guiLeft + xOffset + i * 16 + 11, this.guiTop + yOffset - stats.strength, 0, textureSize - stats.strength, 3, stats.strength);
