...
public class BlockModPlant extends BlockCrops implements IAgriCraftPlant {
	...
	/**
	 * Parameters can be given in any order, parameters can be: @param args: Arguments can be given in any order, parameters can be: String name (needed) ItemStack fruit(needed) BlockWithMeta soil (optional, pass this argument before the RequirementType, else it will be interpreted as a baseblock) RequirementType (optional) BlockWithMeta baseBlock (optional, only if a RequirementType is specified first, else it will be set a a soil) int tier (necessary) RenderMethod renderType (necessary) ItemStack shearDrop (optional, first ItemStack argument will be the regular fruit, second ItemStack argument is the shear drop) Will throw MissingArgumentsException if the needed arguments are not given. This constructor creates the seed for this plant which can be gotten via blockModPlant.getSeed(). This constructor also registers this block and the item for the seed to the minecraft item/block registry and to the AgriCraft CropPlantHandler.
	 */
	...
	public ItemStack getRandomFruit(Random rand) {
		List<ItemStack> fruits = this.getFruit(1, rand);
		return fruits.size() > 0 ? fruits.get(0) : null;
	}

	private List<ItemStack> getFruit(int nr, Random rand) {
