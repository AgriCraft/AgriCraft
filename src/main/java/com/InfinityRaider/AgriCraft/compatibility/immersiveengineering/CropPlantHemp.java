...
public class CropPlantHemp extends AgriCraftPlantTallGeneric {

	private static final Block plant = (Block) Block.blockRegistry.getObject("ImmersiveEngineering:hemp");
	private static final Item seed = (Item) Item.itemRegistry.getObject("ImmersiveEngineering:seed");
	private static final Item fruit = (Item) Item.itemRegistry.getObject("ImmersiveEngineering:material");

	public CropPlantHemp() {
	...
	public IIcon getPlantIcon(int growthStage) {
		if (growthStage > maxMetaBottomBlock()) {
			return plant.getIcon(0, 5);
		}
		if (growthStage > 4) {
			return plant.getIcon(0, 3);
		}
		if (growthStage > 2) {
			return plant.getIcon(0, 2);
		}
		if (growthStage > 0) {
			return plant.getIcon(0, 1);
		}
