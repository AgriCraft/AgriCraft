...
public class RenderBlockGrate extends RenderBlockCustomWood<TileEntityGrate> {
    ...
    protected boolean doWorldRender(TessellatorV2 tessellator, IBlockAccess world, double x, double y, double z, TileEntity tile, Block block, float f, int modelId,boolean callFromTESR) {
        ...
        if(orientation == 0) {
            ...
            drawScaledPrism(tessellator, 1, 0, 0, 3, 16, 2, cm);
            drawScaledPrism(tessellator, 5, 0, 0, 7, 16, 2, cm);
            drawScaledPrism(tessellator, 9, 0, 0, 11, 16, 2, cm);
            drawScaledPrism(tessellator, 13, 0, 0, 15, 16, 2, cm);
            drawScaledPrism(tessellator, 0, 1, 0, 16, 3, 2, cm);
            drawScaledPrism(tessellator, 0, 5, 0, 16, 7, 2, cm);
            drawScaledPrism(tessellator, 0, 9, 0, 16, 11, 2, cm);
            drawScaledPrism(tessellator, 0, 13, 0, 16, 15, 2, cm);
            ...
            drawScaledPrism(tessellator, 0, 0, 1, 2, 16, 3, cm);
            drawScaledPrism(tessellator, 0, 0, 5, 2, 16, 7, cm);
            drawScaledPrism(tessellator, 0, 0, 9, 2, 16, 11, cm);
            drawScaledPrism(tessellator, 0, 0, 13, 2, 16, 15, cm);
            drawScaledPrism(tessellator, 0, 1, 0, 2, 3, 16, cm);
            drawScaledPrism(tessellator, 0, 5, 0, 2, 7, 16, cm);
            drawScaledPrism(tessellator, 0, 9, 0, 2, 11, 16, cm);
            drawScaledPrism(tessellator, 0, 13, 0, 2, 15, 16, cm);

            tessellator.addTranslation(-offset, 0, 0);
        } else {
            tessellator.addTranslation(0, offset, 0);

            drawScaledPrism(tessellator, 0, 0, 1, 16, 2, 3, cm);
            drawScaledPrism(tessellator, 0, 0, 5, 16, 2, 7, cm);
            drawScaledPrism(tessellator, 0, 0, 9, 16, 2, 11, cm);
            drawScaledPrism(tessellator, 0, 0, 13, 16, 2, 15, cm);
            drawScaledPrism(tessellator, 1, 0, 0, 3, 2, 16, cm);
            drawScaledPrism(tessellator, 5, 0, 0, 7, 2, 16, cm);
            drawScaledPrism(tessellator, 9, 0, 0, 11, 2, 16, cm);
            drawScaledPrism(tessellator, 13, 0, 0, 15, 2, 16, cm);
    ...
    private void drawVines(TessellatorV2 tessellator, boolean front, float offset, int orientation) {
        ...
        if(orientation == 0) {
            drawScaledFaceDoubleXY(tessellator, 0, 0, 16, 16, pos);
        }
        else if(orientation == 1) {
            drawScaledFaceDoubleYZ(tessellator, 0, 0, 16, 16, pos);
        }
        else {
            drawScaledFaceDoubleXZ(tessellator, 0, 0, 16, 16, pos);
        }
    }
