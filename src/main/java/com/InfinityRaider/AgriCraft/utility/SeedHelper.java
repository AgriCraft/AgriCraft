...
public abstract class SeedHelper {
	...
	public static double getSpreadChance(Item seed, int meta) {
		...
		AgriCraftPlantDelegate plant = CropPlantHandler.getPlantFromStack(new ItemStack(seed, 1, meta));
		if (plant == null) {
		return 0;
		}
	...
	public static ItemStack getRandomSeed(Random rand, boolean setTag, List<AgriCraftPlantDelegate> plants) {
		boolean flag = false;
		ItemStack seed = null;
		while (!flag) {
		AgriCraftPlantDelegate plant = plants.get(rand.nextInt(plants.size()));
