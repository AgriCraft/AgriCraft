...
public class MagnifyingGlassSoilInspector implements IMagnifyingGlassInspector, IRenderUtilities {
    ...
    protected void drawSoilPropertyTexture(PoseStack transforms, ResourceLocation texture, float x, float w, float u1, float u2) {
        ...
        RenderType renderType = SoilPropertyRenderType.getRenderType(texture);
        ...
        float v1 = 0.0F;
        float v2 = 1.0F;
        builder.vertex(matrix, x1, y2, 0).color(1.0F, 1.0F, 1.0F, 1.0F).uv(u1, v2).endVertex();
        builder.vertex(matrix, x2, y2, 0).color(1.0F, 1.0F, 1.0F, 1.0F).uv(u2, v2).endVertex();
        builder.vertex(matrix, x2, y1, 0).color(1.0F, 1.0F, 1.0F, 1.0F).uv(u2, v1).endVertex();
        builder.vertex(matrix, x1, y1, 0).color(1.0F, 1.0F, 1.0F, 1.0F).uv(u1, v1).endVertex();
    }
    ...
    public static class SoilPropertyRenderType extends RenderType {
        ...
        public SoilPropertyRenderType(String name, VertexFormat format, VertexFormat.Mode mode, int bufferSize, boolean crumbling, boolean sorted, Runnable runnablePre, Runnable runnablePost) {
        ...
        public static RenderType getRenderType(ResourceLocation texture) {
            return RENDER_TYPES.computeIfAbsent(texture, rl ->
                    RenderType.create("tex_" + rl.getNamespace() + "_" + rl.getPath(),
