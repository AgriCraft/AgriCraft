...
public interface IAgriRegistry<T extends IAgriRegisterable> {
     ...
     * @return {@literal true} if an element has been registered to the registry
     * with the given id, {@literal false} otherwise.
     */
     ...
     * @return {@literal true} if the registry has an exact registration for the
     * given element.
     */
    boolean has(@Nullable T element);

    /**
     * Fetches the element in the registry with the given id, or else returns
     * the empty optional.
     *
     * @param id the id of the element in the registry to be fetched.
     * @return the element with the given id in the registry, or the empty
     * optional.
     */
    @Nonnull
     ...
     * @return {@literal true} if the element was registered, {@literal false}
     * otherwise (i.e. in the case of a conflict).
     */
     ...
     * @return {@literal true} if the element was removed, {@literal false}
     * otherwise (i.e. in the case that the element was not actually in the
     * registry).
     */
     ...
     * <p>
     * Notice, this method should probably be avoided, in favor of using
     * {@link #stream()}, given that the latter method can directly connect to
     * the backing collection without having to wrap it.
     *
