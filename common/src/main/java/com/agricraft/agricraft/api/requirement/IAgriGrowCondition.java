...
public interface IAgriGrowCondition {
	 ...
	 * @return the response.
	 */
	IAgriGrowthResponse check(IAgriCrop crop, @NotNull Level world, @NotNull BlockPos pos, int strength);

	/**
	 * @return a set of all fluid positions, relative to the crop which need to be checked for this condition, can be empty (e.g in case of dimension)
	 */
	 ...
	 */
	void notMetDescription(@NotNull Consumer<Component> consumer);
	 ...
	 * - BLOCK_UPDATE: cache value changes when a fluid update happens; isMet() is called after a fluid update only
	...
	enum CacheType {
		...
		/**
		 * Block update caching; examples include "things" which stay the same until a fluid update happens:
		 * - Presence of a certain fluid at a certain position
		 * - Presence of a liquid inside the fluid
		 * - etc.
		 */
		BLOCK_UPDATE,
